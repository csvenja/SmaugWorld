...
int main() {
	...
	*mealWaitingFlagp = 0;
	...
	while (TRUE) {
		...
		/* wait for processes that have exited so we do not accumulate zombie or
		 * cows */
		...
		while (*mealWaitingFlagp >= 1) {
			*mealWaitingFlagp = *mealWaitingFlagp - 1;
			...
			if (cowsEatenTotal >= MAX_COWS_EATEN) {
				printf("SMAUGSMAUGSMAUGSMAUGSMAU   Smaug has eaten the allowed number "
							 "of cows\n");
				*terminateFlagp = 1;
				break;
			}
			...
			if (*mealWaitingFlagp > 0) {
				...
				semopChecked(semID, &SignalProtectMealWaitingFlag, 1);
				printf(
						"SMAUGSMAUGSMAUGSMAUGSMAU   Smaug sleeps again\n"); //, localpid);
				semopChecked(semID, &WaitDragonSleeping, 1);
...
void initialize() {
	...
	if ((terminateFlag = shmget(IPC_PRIVATE, sizeof(int), IPC_CREAT | 0666)) <
			0) {
...
void releaseSemandMem() {
	...
	if (shmctl(mealWaitingFlag, IPC_RMID, NULL)) {
		printf(
				"RELEASERELEASERELEAS   mealWaitingFlag share memory delete failed \n");
	} else {
